...
fn show_menu() -> (usize, i32) {
    let modetext = ["Marathon (default)", "Sprint 40 lines   ", "Ultra 3 min        "];
    let mut mode = 0;
    let mut speed = 2;
    let mut menu_page = 0;
    ...
    loop {
        ...
        if menu_page == 0 {
            ...
            eadk::display::draw_string("[Options]", Point { x: 10, y: 10 }, false, COLOR_WHITE, COLOR_BLACK);
            eadk::display::draw_string("Mode : ", Point { x: 30, y: 40 }, false, COLOR_WHITE, COLOR_BLACK);
            
            if mode != 0 {
                eadk::display::draw_string("Speed : ", Point { x: 30, y: 60 }, false, COLOR_WHITE, COLOR_BLACK);
                let speed_str = format!("{}   ", speed);
                eadk::display::draw_string(&speed_str, Point { x: 120, y: 60 }, false, COLOR_WHITE, COLOR_BLACK);
            } else {
                eadk::display::draw_string("               ", Point { x: 30, y: 60 }, false, COLOR_WHITE, COLOR_BLACK);
            }
            eadk::display::draw_string(modetext[mode], Point { x: 110, y: 40 }, false, COLOR_WHITE, COLOR_BLACK);
            ...
            let mut sm_change = 0;
            if keys.key_down(Key::Up) && speed < 15 {
                sm_change += 1;
            }
            if keys.key_down(Key::Down) && speed > 1 {
                sm_change -= 1;
            }
            if sm_change != 0 {
                speed += sm_change;
                eadk::timing::msleep(120);
            } else {
                eadk::timing::msleep(30);
            }
            
            if eadk::input::KeyboardState::scan().key_down(Key::Zero) {
                menu_page = 0;
                draw_logo();
                while eadk::input::KeyboardState::scan().key_down(Key::Zero) {
                    eadk::timing::msleep(10);
...
fn run_game(mode: usize, speed: i32) -> GameOutcome {
    let mut x = 140;
    let mut y = 20;
    let mut px = 150;
    let mut py = 10;
    ...
    let mut score = 0;
    let mut combo = 0;
    let mut level = if mode == 0 { 1 } else { speed };
    let mut xp = 0;
    let mut btob = 0;
    let mut lines = 0;
    let mut end = false;
    ...
    if mode == 0 {
        eadk::display::draw_string("01", Point { x: 65, y: 180 }, false, COLOR_WHITE, COLOR_BLACK);
    } else {
...
fn main() -> isize {
    ...
    loop {
        let (mode, speed) = show_menu();
        loop {
            let outcome = run_game(mode, speed);
